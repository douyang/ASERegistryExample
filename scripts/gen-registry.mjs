...
// "evaluation on the echo registry" for every product family in data/products.js.
...
const BRAND = JSON.parse(fs.readFileSync(path.join(root, 'config', `brand.${fs.existsSync(path.join(root, 'data', 'brand.js')) ? (/Active branding profile: (\w+)/.exec(fs.readFileSync(path.join(root, 'data', 'brand.js'), 'utf8')) || [, 'neutral'])[1] : 'neutral'}.json`), 'utf8'));
const REG_NAME = BRAND.registry.name;
...
// Real registry facts (for context on the page). Sources: JASE 2025 (PMC11779749). The neutral
// branding profile hides this block, because the facts identify a specific registry.
const REGISTRY = {
  name: REG_NAME,
...
const out = {
  simulated: true,
  notice: `SIMULATED DATA. Every score in this file is generated by scripts/gen-registry.mjs from a fixed seed. No product has been evaluated on ${REG_NAME}. Products are labelled anonymously so no simulated score is attached to a real device. Facts under "registry" are real and sourced.`,
