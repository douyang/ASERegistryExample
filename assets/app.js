...
(function () {
  ...
  function chips(f) {
    ...
    for (const t of (f.tags || []).slice(0, 4)) out.push(`<span class="chip">${esc(t)}</span>`);
    return out.join('');
  }
  ...
  function panelHTML(f) {
    ...
    const feats = (f.embedded_ai_features || []);
    ...
    return `
      <div class="panel-head">
        ...
        <div class="chips">${(f.modality_scope || []).map((m) => `<span class="chip">${esc(m)}</span>`).join('')}${(f.deployment || []).map((m) => `<span class="chip">${esc(m)}</span>`).join('')}${(f.tags || []).map((t) => `<span class="chip on">${esc(t)}</span>`).join('')}</div>
