...
Catalog of FDA-cleared AI echocardiography products, with a second tab that scores each product against an echo registry’s adult TTE module and a third reporting per-site quality.
...
config/brand.*.json        branding profiles: neutral (public) and ase
data/brand.js              GENERATED active profile (scripts/set-brand.mjs)
...
scripts/set-brand.mjs      selects the branding profile -> data/brand.js
...
The site ships **de-branded**. `config/brand.neutral.json` is the committed default and carries no organisation logo, name, attribution or registry identity: a plain mark, the tagline "FDA-cleared AI echocardiography products", no footer attribution, the registry referred to only as "the registry", and the society-specific registry-facts block hidden.

`config/brand.ase.json` turns the full American Society of Echocardiography / ImageGuideEcho Registry identity back on — gradient mark, masthead line, footer attribution with links, registry name in every heading, and the sourced registry facts.

```
node scripts/set-brand.mjs neutral   # public default
node scripts/set-brand.mjs ase       # society identity on
node scripts/gen-registry.mjs        # the registry name is baked into generated data
